...
complete -c qemu -o no-hpet -d 'Disable HPET support.  Deprecated, use \\(aq-machine hpet=off\\(aq instead'
...
complete -c qemu -o semihosting-config -d 'Enable and configure \\%Semihosting (ARM, M68K, Xtensa, MIPS, Nios II, RISC-V …'
...
complete -c qemu -o async-teardown -d 'This option is deprecated and should no longer be used'
complete -c qemu -o run-with -d 'Set QEMU process lifecycle options'
...
complete -c qemu -o perfmap -d 'Generate a map file for Linux perf tools that will allow basic profiling info…'
complete -c qemu -o jitdump -d 'Generate a dump file for Linux perf tools that maps basic blocks to symbol na…'
