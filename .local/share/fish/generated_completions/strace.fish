...
complete -c strace -l argv0 -d 'Set argv[0] of the command being executed to R name '
...
complete -c strace -o einject -d 'TQ --inject=\\,syscall_set\\/[:error=\\,errno\\/|:retval=\\,value\\/]\\:[:signal=\\,s…'
...
complete -c strace -l status -d 'options are not considered when keeping track of the number of syscalls that …'
complete -c strace -l trace-fds -d 'Trace only the syscalls that operate on the specified subset of (non-negative…'
...
complete -c strace -l version -d 'Print the version number of strace '
